/**
 * The README (and server.json, the highest-return text in the project — it
 * is what a registry listing indexes) are marketing copy that quotes
 * registry facts. Prose drifts; the registry does not. This test makes a
 * stale claim fail `prebuild` instead of shipping — the counts are what
 * feed server.json and every directory listing.
 */
...
describe("server.json claims match the registry", () => {
  it("quotes the real component total in its description", () => {
    const claims = [
      ...SERVER_JSON.description.matchAll(/([\d,]+) copy-paste/g),
    ].map((m) => m[1]);
    expect(claims.length).toBeGreaterThan(0);
    const expected = totalCount.toLocaleString("en-US");
    for (const claim of claims) expect(claim).toBe(expected);
